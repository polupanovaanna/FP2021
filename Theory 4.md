...
Пояснение: Посмотрим заново на структуру списка. Это всё работает так, что вместо `c` подставляется `tru`, вместо `n` подставляется `undefined`(т.к. никак оно не используется). И при раскрытии списка всегду будет браться внешний элемент.
