...
Из арифметических операций нам понадобится определить `++, --, +, -, *, ^`
...
Как мне сообщили, что следующее тоже хорошо понимать:
`Про \x y -> y x`

Эта штука не типизируется (теми типами, которые мы хотим от чисел чёрча), поэтому не выражается напрямую через семантику "применить функцию n раз". 
Можно попробовать убедиться на паре примеров и/или доказать по индукции, но это довольно муторно, и не требуется.

То есть доказательство существенно опирается на конкретное представление чисел, которое мы используем, а не просто на то, что n f x имеет семантику "примени f к x n раз".
